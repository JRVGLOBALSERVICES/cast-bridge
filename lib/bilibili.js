...
async function expandShort(urlStr) {
  try {
    /* safeFetch's second argument is an Accept header, not an options bag —
       this used to pass {method:'HEAD'} there, which never reached fetch. It
       has always been a GET, and it has to be: bili.im, the sibling
       shortener, answers 405 to a HEAD. */
    const { url } = await safeFetch(urlStr, 'text/html');
    return url;
  } catch (e) {
    return null;
  }
...
async function resolve(urlStr, cookie) {
  ...
  if (target.short) {
    ...
    if (!target || !target.bvid) {
      /* A b23.tv share can land on bilibili.tv, which is a different site
         with a different resolver. Handing the long form back is api/extract's
         cue to try that one; calling it "not a video" was wrong about a link
         that plays perfectly well one module over. */
      return { ok: false, handoff: long, error: 'That b23.tv link is not a video.' };
    }
  }
  ...
  if (!pages.body || pages.body.code !== 0 || !Array.isArray(pages.body.data) || !pages.body.data.length) {
    return {
      ok: false,
      error: 'Bilibili would not say what is on that page' +
        (pages.body && pages.body.code != null ? ' (code ' + pages.body.code + ')' : '') + '.'
    };
  }
  ...
  if (!play.body || play.body.code !== 0 || !play.body.data) {
    ...
    /* Not `'Bilibili said: ' + msg`. The upstream message is written for
       Bilibili's own operators and arrives in Chinese — measured on the .tv
       side, where a refusal came back as Chinese text about activity tags
       that had nothing to do with the request. Forwarding that verbatim puts
       a sentence in front of the reader that cannot help them. The code is
       carried instead: it can be looked up, and it is honest about the fact
       that we do not know what it means. */
    return {
      ok: false,
      error: 'Bilibili would not give an address for it' +
        (play.body && play.body.code != null ? ' (code ' + play.body.code + ')' : '') + '.'
    };
  }
...
function diagnoseCookieText(text) {
  ...
  if (tv && !com) {
    return 'That export is from bilibili.tv, which is a separate sign-in from ' +
      'bilibili.com and belongs under Bilibili TV rather than here. Paste it ' +
      'there instead — or if you meant the mainland site, sign in at ' +
      'bilibili.com and export again.';
  }
  if (com) {
