...
/* OpenSubtitles' keyless door refuses one request and answers the next: on
   2026-09-15 a search and a download each came back 502 and the same tap a
   few seconds later worked. So a refusal that says "busy" (429, 5xx) or a
   dropped connection is tried once more before the phone hears about it.
   A timeout is not retried — two of those would outlast the function. */
...
async function timedRetry(fetchImpl, url, init, wait) {
  const pause = wait == null ? RETRY_DELAY_MS : wait;
  let res;
  try {
    res = await timed(fetchImpl, url, init);
  } catch (e) {
    if (e && e.name === 'AbortError') throw e;
    await new Promise((r) => setTimeout(r, pause));
    return timed(fetchImpl, url, init);
  }
...
async function search(input, env, fetchImpl) {
  ...
  try {
    const res = await timedRetry(f, legacySearchUrl(q), { headers: { 'User-Agent': UA, 'X-User-Agent': UA, Accept: 'application/json' } }, (env || {}).retryDelayMs);
...
async function download(ref, encoding, env, fetchImpl) {
  ...
  if (r.door === 'os') {
    ...
    for (let hop = 0; hop < 3; hop++) {
      res = await timedRetry(f, url, { headers: { 'User-Agent': UA } }, (env || {}).retryDelayMs);
