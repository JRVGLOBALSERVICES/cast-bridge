...
async function rescan(pageUrl, wantUrl) {
  /* Keyed by the site wanted as well as the page: a page that embeds four
     players answers differently depending on which one refused us, and a
     walk that stopped at the first player with an address must not be
     handed back to a caller that wanted the fourth. */
  let wantSite = '';
  try { wantSite = wantUrl ? new URL(wantUrl).hostname : ''; } catch (e) { wantSite = ''; }
  const cacheKey = 'page:' + pageUrl + (wantSite ? '#' + wantSite : '');
  const cached = cacheGet(cacheKey);
  if (cached !== undefined) return cached;
  /* A plain walk of the same page that already holds the wanted site is a
     complete answer; one that does not is the stale half described above. */
  const plain = wantSite ? cacheGet('page:' + pageUrl) : undefined;
  if (plain !== undefined && plain.some((x) => sameSite(x.url, wantUrl))) return plain;
  ...
  try {
    ...
    if (!res.ok || !readable(res)) {
      ...
      const found = () => wantUrl
        ? media.some((x) => sameSite(x.url, wantUrl))
        : media.length > 0;
      ...
      if (!found()) {
        ...
        for (const frameUrl of frames) {
          ...
          /* Measured 17 Sep 2026: 1tamilcrow embeds Dailymotion, Vimeo,
             then the player that actually served the film. Stopping at the
             first frame with any address re-issued Dailymotion and left the
             refused film refused. */
          if (found()) break;
        }
      }
    }
...
/* Why a re-issue came back empty, in words a log reader can act on: the
   hosts it DID find say whether the page changed player or just hid it. */
function noPick(step, media, failedUrl) {
  let host = '?';
  try { host = new URL(failedUrl).hostname; } catch (e) { /* unparseable */ }
  if (!media || !media.length) return step + ' found no stream on the page';
  const hosts = Array.from(new Set(media.map((x) => {
    try { return new URL(x.url).hostname; } catch (e) { return '?'; }
  }))).slice(0, 4).join(', ');
  const same = media.some((x) => x.url === failedUrl);
  return step + (same
    ? ' found only the address that was already refused'
    : ' found ' + media.length + ' stream(s) on ' + hosts + ', none from ' + host);
}
...
async function reissue(pageUrl, failedUrl, label, note) {
  const why = typeof note === 'function' ? note : () => {};
  ...
  if (sameHost(pageUrl, failedUrl)) { why('page re-read skipped: the page and the stream are on one host'); return null; }

  const media = await rescan(pageUrl, failedUrl);
  const got = choose(media, failedUrl, label);
  if (!got) { why(noPick('page re-read', media, failedUrl)); return null; }
...
async function deepReissue(pageUrl, failedUrl, label, note) {
  const why = typeof note === 'function' ? note : () => {};
  if (!pageUrl || !failedUrl) return null;
  if (!/^https?:\/\//i.test(pageUrl)) return null;
  if (!deepAvailable()) { why('browser re-scan off: no CHROME_EXECUTABLE_PATH on this host'); return null; }
  if (sameHost(pageUrl, failedUrl)) { why('browser re-scan skipped: the page and the stream are on one host'); return null; }

  const media = await deepRescan(pageUrl);
  const got = choose(media, failedUrl, label);
  if (!got) { why(noPick('browser re-scan', media, failedUrl)); return null; }
  return { url: got.url, referer: got.referer || pageUrl };
}
