...
/* ------------------------------------------------------------------ *
 * A piece of a stream is not a stream
 *
 * An HLS playlist is a list of segments, and a player fetches every one of
 * them. Watching a page's network therefore turns up the film once — as a
 * .m3u8 — and then several hundred times more as six-second fragments of
 * it. Offering those as things to play is offering someone six seconds of
 * their episode, and it is what put three rows on a tvarticles.org page
 * where there were two streams.
 *
 * The naming is not a tell and cannot be made into one. tvlogy.to calls its
 * segments `01852-000.juicycodes`; another host will call them something
 * else tomorrow. So the test is not what a segment is CALLED. It is, in
 * order of how much the evidence is worth:
 *
 *   1. A playlist we read listed it. That is not an inference — the file
 *      the player treats as a list of parts says which parts.
 *   2. It is served as video/mp2t. A transport stream is a container for a
 *      fragment; nothing hands a player a whole film in one.
 *   3. Its name ends in a segment extension.
 *   4. Its kind is not one any player opens on its own.
 *
 * The fourth rule is the one that has to stay generous. A signed CDN link
 * carries no extension at all, and `kindOf` calls that VIDEO — refusing it
 * would throw away the addresses that are hardest to come by and easiest to
 * get wrong, which is the opposite of the point.
 * ------------------------------------------------------------------ */

/* Only ever a fragment. video/mp4 is NOT here: CMAF segments use it, and so
   does every whole mp4 on the web, so the type cannot separate them — rules
   1 and 3 do that. */
const SEGMENT_TYPE = /^(video\/mp2t|video\/mpts|application\/x-mpegts|video\/iso\.segment)$/i;

/* Segment extensions. `.aac` and `.mp3` are deliberately absent — those are
   ordinary files someone may well want to play on their own. */
const SEGMENT_EXT = /\.(ts|m4s|cmfv|cmfa|cmft|fmp4|init)(\?|$)/i;

/* Extensions a player opens when handed nothing else. */
const PLAYABLE_EXT =
  /^(m3u8|mpd|mp4|m4v|webm|mkv|mov|ogv|mp3|m4a|aac|ogg|opus|flac|wav)$/i;

/* A body that has named itself a whole container. This is the only evidence
   that outranks the address, and it has to: a great many of these sites
   serve their film from `get_file.php`, and a rule that read the suffix and
   stopped would refuse a stream we had already proved by reading it.

   octet-stream is deliberately absent. Half the CDNs on the web label an
   mp4 that way and the other half label a font that way, so it proves
   nothing on its own and the address gets to decide. */
const CONTAINER_TYPE =
  /^(video\/(mp4|webm|x-matroska|quicktime|ogg|x-m4v|mp4v-es)|audio\/|application\/(vnd\.apple\.mpegurl|x-mpegurl|dash\+xml))/i;

/* What probeMedia proved, said in the language of a content type, so a
   proven body and a declared one go through the same gate. */
const KIND_TYPE = {
...
function kindToType(kind) {
  return KIND_TYPE[String(kind || '').toUpperCase()] || '';
}

/* Every address a playlist points at, and whether the playlist was a master.
 *
 * A master playlist's URIs are other playlists — those ARE worth offering,
 * which is what expandHlsMaster exists to do. A media playlist's URIs are
 * segments and none of them is. Telling the two apart is one tag. */
function playlistLinks(text, baseUrl) {
  const body = String(text || '');
  const master = /#EXT-X-STREAM-INF/i.test(body);
  const urls = [];

  const push = (raw) => {
    const abs = absolute(String(raw || '').trim(), baseUrl);
  ...
  body.split(/\r?\n/).forEach((line) => {
    ...
    if (!s) return;
    if (s.charAt(0) === '#') {
      /* An initialisation segment is named in a tag rather than on a line
         of its own, and it is as much a fragment as any other. */
      const m = /#EXT-X-MAP:[^\n]*URI\s*=\s*"([^"]+)"/i.exec(s);
      if (m) push(m[1]);
      return;
    }
...
/* The one gate. `segments` is a Set of addresses a playlist claimed, and is
   optional — the other rules stand on their own without it. */
function offerable(url, contentType, segments) {
  const u = String(url || '');
  if (!/^https?:\/\//i.test(u)) return false;
  if (segments && segments.has(u)) return false;

  const type = String(contentType || '').split(';')[0].trim().toLowerCase();
  if (type && SEGMENT_TYPE.test(type)) return false;

  const name = u.split('?')[0].split('#')[0].split('/').pop() || '';
  if (SEGMENT_EXT.test(name)) return false;
  if (NOT_MEDIA_EXT.test(name)) return false;

  /* Proved by its own body — the address no longer gets a vote. */
  if (type && CONTAINER_TYPE.test(type)) return true;

  /* An address that ends in something extension-shaped has to end in one a
     player knows. `01852-000.juicycodes` does not, and that is the whole of
     the reported bug when the response's type never reached us — which is
     the ordinary case, because a segment is captured at REQUEST time, one
     round trip before anything has said what it is.

     An address with no suffix at all is a different thing and is kept: a
     signed CDN link carries no name to be judged on, and those are the
     addresses hardest to come by. */
  const suffix = (name.match(/\.([A-Za-z0-9]{1,10})$/) || [])[1];
  if (suffix) return PLAYABLE_EXT.test(suffix);
  return true;
}
...
async function probeAll(candidates, detail) {
  ...
  settled.forEach((hit) => {
    ...
    /* The same gate the deep scan applies. A player config can name a
       segment as readily as a page's network can serve one, and a rule that
       only guards one of the two doors is not a rule. */
    if (!offerable(hit.url, kindToType(hit.kind))) return;
    seen.add(hit.url);
    out.push({
      url: hit.url,
      kind: hit.kind,
...
module.exports = {
  ...
  SEGMENT_TYPE, SEGMENT_EXT, PLAYABLE_EXT, CONTAINER_TYPE,
  kindToType, playlistLinks, offerable,
