...
const TYPES = new Set(['load', 'page', 'play', 'pause', 'seek', 'skip', 'stop', 'mute']);
const MIMES = new Set([
...
function shapeCommand(body) {
  ...
  /* A web page, not a film: the TV's own browser opens it and the site's
     player runs there. Sent from the in-app Browser. */
  if (type === 'page') {
    const url = httpUrl(body.url);
    if (!url) throw new InvalidTv('No page to open.');
    return { type, url, title: text(body.title, MAX_TEXT) };
  }
  if (type === 'seek') return { type, to: seconds(body.to) };
  if (type === 'skip') return { type, by: seconds(body.by, { signed: true }) };
  /* on: true/false sets it; anything else toggles whatever the TV has now. */
  if (type === 'mute') return { type, on: typeof body.on === 'boolean' ? body.on : null };
  return { type };
}
